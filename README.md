...
<p align="center">
  <strong>一条龙网盘媒体自动化平台：从找资源、自动转存、智能整理、挂载 STRM、到 Emby 302 直链播放，全流程打通；重点强化 115 / 123 / 光鸭云盘整理与转存、115 / 123 / 光鸭 STRM 全量与增量生成、以及 Emby 反向代理 302 播放能力。</strong>
</p>
...
### 1. 📦 115 / 123 / 光鸭网盘整理功能是核心主线能力

项目当前最值得强调的能力，不是“单点转存”，而是 **115、123 与光鸭的影视库自动整理**：
...
* **115 / 123 / 光鸭多平台覆盖**：不是只有 123，也不是只有 115，光鸭云盘也已接入整理链路。

一句话总结：**把“转存来的资源”自动变成“像样的影视库”。**

### 2. 🎞️ 115 / 123 / 光鸭 STRM 全量 + 增量生成能力

项目第二条主线能力，是 **115 / 123 / 光鸭多平台 STRM 输出能力**：

* **123 STRM 全量生成**：按目录扫描批量生成 `.strm`，STRM 采用 fileid 类型，疾速请求直链。
* **115 STRM 全量生成**：与 123 同样支持整库扫描输出，STRM 采用 pickcode 类型，疾速请求直链。
* **光鸭 STRM 全量生成**：按光鸭目录生成 `.strm`，STRM 采用 `/playgy/{file_id}` 类型，播放时实时获取签名直链。
...
* **Telegram 频道监控**：支持 123 / 115 / 光鸭 / 天翼 / 影巢频道自动监听与转存。
* **榜单订阅**：支持豆瓣榜单、猫眼榜单自动参与转存判断。
* **关键词白名单 / 黑名单 / 二次分类规则**：对转存触发条件进行精细控制。
* **光鸭分享转存**：支持光鸭手机号登录 Token、分享链接转存、频道监控转存，以及分享目录兜底展开处理。
...
2. 打开「115 账号配置」「123 账号配置」「光鸭云盘 - 登录与 Token」「天翼账号配置」，补齐要使用的网盘账号。
...
用于通过手机号短信验证码登录光鸭云盘，登录成功后会持久化保存 access token 与 refresh token，供光鸭分享转存、频道监控和网盘整理使用。

![光鸭云盘登录与 Token](picture/光鸭云盘-登录与Token.png)

#### 频道监控配置

用于配置光鸭资源频道监听规则，支持公开频道和 TG API 频道 ID，结合关键词白名单、黑名单、豆瓣 / 猫眼榜单和二次分流规则，把光鸭分享自动转存到指定目录。

![光鸭云盘频道监控配置](picture/光鸭云盘-频道监控配置.png)
...
用于扫描光鸭云盘影视目录，按 TMDB、媒体信息和 AI 识别结果完成分类、重命名、归档和洗版，配置项与 123 / 115 整理链路保持一致。

![光鸭云盘网盘整理功能](picture/光鸭云盘-网盘整理功能.png)

#### STRM 生成

用于为光鸭云盘目录生成 STRM 文件，播放路径使用 `/playgy/`，可接入 Emby、Jellyfin、Plex 等媒体库。
...
确认 STRM 播放基础地址能被 Emby 服务器访问；确认 115 / 123 / 光鸭账号仍有效；确认 Emby 反代实例已运行；确认播放器访问的端口没有被防火墙阻断。
